...
"""A Code Intelligence Language Engine for the Wuzzy language.
...
Module Usage:
    from cile_wuzzy import scan
    mtime = os.stat("bar.wuzzy")[stat.ST_MTIME]
    content = open("bar.wuzzy", "r").read()
    scan(content, "bar.wuzzy", mtime=mtime)
"""
...
class WuzzyCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.wuzzy")
...
def scan_buf(buf, mtime=None, lang="Wuzzy"):
    """Scan the given WuzzyBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {WuzzyBuffer} is the Wuzzy buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Wuzzy CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Wuzzy content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Wuzzy <example-Wuzzy-file>
    ...
    # Dev Note:
    # This is where you process the Wuzzy content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Wuzzy-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Wuzzy content, you may
    #   want to use it.
