...
lang = "Wuzzy"
log = logging.getLogger("codeintel.wuzzy")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Wuzzy-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "apple",
...
# lexers. You must define a Lexer class for lang Wuzzy. If Komodo's
# scintilla lexer for Wuzzy is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class WuzzyLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Wuzzy is one of Komodo's existing C++ lexers
...
#   class WuzzyLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_WUZZY)
#           self._keyword_lists = [
...
# examples of usage.
class WuzzyLangIntel(LangIntel):
    lang = lang
...
# Wuzzy content and returns a description of the code in that file.
# See "cile_wuzzy.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Wuzzy is
...
class WuzzyCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_wuzzy
        return cile_wuzzy.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=WuzzyLexer(),
        buf_class=WuzzyBuffer,
        langintel_class=WuzzyLangIntel,
        import_handler_class=None,
        cile_driver_class=WuzzyCILEDriver,
