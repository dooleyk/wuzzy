# Registers the Wuzzy language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koWuzzyLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Wuzzy")
    registry.registerLanguage(KoWuzzyLanguage())

class KoWuzzyLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Wuzzy"
    lexresLangName = "Wuzzy"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "077a1e5f-d42d-4b66-a388-07d43aed8170"

    defaultExtension = '.wzy'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Wuzzy'}
